/**
 * generated by Xtext 2.26.0
 */
package xtext.cNNDescLang;

import org.eclipse.emf.ecore.EFactory;

/**
 * <!-- begin-user-doc -->
 * The <b>Factory</b> for the model.
 * It provides a create method for each non-abstract class of the model.
 * <!-- end-user-doc -->
 * @see xtext.cNNDescLang.CNNDescLangPackage
 * @generated
 */
public interface CNNDescLangFactory extends EFactory
{
  /**
   * The singleton instance of the factory.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  CNNDescLangFactory eINSTANCE = xtext.cNNDescLang.impl.CNNDescLangFactoryImpl.init();

  /**
   * Returns a new object of class '<em>CNN Desc Lang</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>CNN Desc Lang</em>'.
   * @generated
   */
  CNNDescLang createCNNDescLang();

  /**
   * Returns a new object of class '<em>Architecture</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Architecture</em>'.
   * @generated
   */
  Architecture createArchitecture();

  /**
   * Returns a new object of class '<em>Convolution</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Convolution</em>'.
   * @generated
   */
  Convolution createConvolution();

  /**
   * Returns a new object of class '<em>Flatten Or Global</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Flatten Or Global</em>'.
   * @generated
   */
  FlattenOrGlobal createFlattenOrGlobal();

  /**
   * Returns a new object of class '<em>Interstice</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Interstice</em>'.
   * @generated
   */
  Interstice createInterstice();

  /**
   * Returns a new object of class '<em>Classification</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Classification</em>'.
   * @generated
   */
  Classification createClassification();

  /**
   * Returns a new object of class '<em>Conv Drop</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Conv Drop</em>'.
   * @generated
   */
  ConvDrop createConvDrop();

  /**
   * Returns a new object of class '<em>Merge Conv</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Merge Conv</em>'.
   * @generated
   */
  MergeConv createMergeConv();

  /**
   * Returns a new object of class '<em>Conv Or Merge</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Conv Or Merge</em>'.
   * @generated
   */
  ConvOrMerge createConvOrMerge();

  /**
   * Returns a new object of class '<em>Left</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Left</em>'.
   * @generated
   */
  Left createLeft();

  /**
   * Returns a new object of class '<em>Right</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Right</em>'.
   * @generated
   */
  Right createRight();

  /**
   * Returns a new object of class '<em>Merge Body</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Merge Body</em>'.
   * @generated
   */
  MergeBody createMergeBody();

  /**
   * Returns a new object of class '<em>Merge</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Merge</em>'.
   * @generated
   */
  Merge createMerge();

  /**
   * Returns a new object of class '<em>Feature Extraction</em>'.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return a new object of class '<em>Feature Extraction</em>'.
   * @generated
   */
  FeatureExtraction createFeatureExtraction();

  /**
   * Returns the package supported by this factory.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the package supported by this factory.
   * @generated
   */
  CNNDescLangPackage getCNNDescLangPackage();

} //CNNDescLangFactory
