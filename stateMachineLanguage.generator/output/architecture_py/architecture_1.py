...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=6, strides=4, activation='selu', padding='valid')(X)
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)
		X = Conv2D(64, kernel_size=5, strides=1, activation='selu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.10)(X)
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(89, kernel_size=2, strides=1, activation='selu', padding='same')(X)
		X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)

		X1 = Conv2D(89, kernel_size=2, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Concatenate()([X, X1])
		X = AveragePooling2D(pool_size=6, strides=1, padding='same')(X)
		X = Conv2D(142, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(142, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(142, kernel_size=2, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.40)(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../architecture_img/architecture_1.png")
	...
	log_file = open( "../architecture_log/architecture_1.log", "w")
	...
	print('OK: file ../architecture_log/architecture_1.log has been create')
...
# something go wrong
	...
	print('error: file ../architecture_log/architecture_1_error.log has been create')
	error_file = open("../architecture_log/architecture_1_error.log" , "w")
