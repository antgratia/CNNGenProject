...
try:
	def getModel():
		...
		X = Conv2D(16, kernel_size=6, strides=5, activation='relu', padding='valid')(X)
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(16, kernel_size=2, strides=1, activation='tanh', padding='same')(X)

		X1 = Conv2D(32, kernel_size=1, strides=1, activation='selu', padding='same')(X1)
		X = Concatenate()([X, X1])
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(54, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		...
		X = Conv2D(44, kernel_size=2, strides=1, activation='relu', padding='same')(X)

		X2 = X
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(44, kernel_size=3, strides=1, activation='tanh', padding='same')(X)

		X = Concatenate()([X, X2])
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)

		X = Concatenate()([X, X1])
		X = Conv2D(90, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)
		X = Conv2D(106, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = Dropout(0.10)(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/tensorboard_2/architecture_4.png")
	model.compile(optimizer='adam', loss=keras.losses.sparse_categorical_crossentropy, metrics=['accuracy'])

	es = EarlyStopping(monitor='loss', min_delta=0.001, verbose=1, restore_best_weights=True, patience=7)
	tb = TensorBoard(log_dir="../../architecture_tb/tensorboard_2/architecture_4")
	list_cb = [es, tb]
	start = time()
	...
	log_file = open( "../../architecture_log/tensorboard_2/architecture_4.log", "w")
	...
	print('OK: file ../../architecture_log/tensorboard_2/architecture_4.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/tensorboard_2/architecture_4_error.log has been create')
	error_file = open("../../architecture_log/tensorboard_2/architecture_4_error.log" , "w")
