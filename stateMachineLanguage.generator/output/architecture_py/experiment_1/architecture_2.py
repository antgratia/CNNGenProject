...
try:
	def getModel():
		...
		X = Conv2D(64, kernel_size=4, strides=4, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(128, kernel_size=3, strides=3, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		...
		X = Dropout(0.20)(X)
		X = Conv2D(512, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)

		X = Flatten()(X)
		X = Dense(92, activation='tanh')(X)
		X = Dense(50, activation='selu')(X)
		X = Dense(31, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_1/architecture_2.png")
	...
	log_file = open( "../../architecture_log/experiment_1/architecture_2.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_1/architecture_2.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_1/architecture_2_error.log has been create')
	error_file = open("../../architecture_log/experiment_1/architecture_2_error.log" , "w")
