...
try:
	def getModel():
		...
		X1 = Conv2D(32, kernel_size=4, strides=1, activation='relu', padding='same')(X1)
		...
		X = Concatenate()([X, X1])
		X = MaxPooling2D(pool_size=6, strides=1, padding='same')(X)
		X = Conv2D(460, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(460, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.20)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)

		X1 = Conv2D(460, kernel_size=2, strides=1, activation='tanh', padding='same')(X1)
		X1 = Conv2D(460, kernel_size=2, strides=1, activation='selu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Concatenate()([X, X1])
		X = Dropout(0.10)(X)
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(1104, kernel_size=1, strides=1, activation='selu', padding='same')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dense(883, activation='tanh')(X)
		X = Dense(335, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../architecture_img/architecture_7.png")
	...
	log_file = open( "../architecture_log/architecture_7.log", "w")
	...
	print('OK: file ../architecture_log/architecture_7.log has been create')
...
# something go wrong
	...
	print('error: file ../architecture_log/architecture_7_error.log has been create')
	error_file = open("../architecture_log/architecture_7_error.log" , "w")
