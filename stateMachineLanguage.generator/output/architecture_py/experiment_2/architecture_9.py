...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
		X = Dropout(0.01)(X)
		X = MaxPooling2D(pool_size=5, strides=3, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(64, kernel_size=6, strides=1, activation='selu', padding='valid')(X)
		...
		X = Dropout(0.20)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='relu', padding='valid')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)

		X1 = X
		X = AveragePooling2D(pool_size=6, strides=1, padding='same')(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X)

		X2 = X1
		X2 = AveragePooling2D(pool_size=3, strides=1, padding='same')(X2)
		X2 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(256, kernel_size=1, strides=1, activation='relu', padding='same')(X2)
		X2 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)

		X3 = X2
		X3 = AveragePooling2D(pool_size=1, strides=1, padding='same')(X3)
		X3 = Conv2D(256, kernel_size=1, strides=1, activation='relu', padding='same')(X3)
		X3 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X3)
		X3 = Dropout(0.80)(X3)
		X3 = MaxPooling2D(pool_size=4, strides=1, padding='same')(X3)

		X2 = Concatenate()([X2, X3])
		X2 = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Dropout(0.50)(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X2)
		...
		X4 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X4)
		...
		X1 = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Concatenate()([X, X1])
		X = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = Dropout(0.80)(X)
		...
		X1 = Conv2D(256, kernel_size=1, strides=1, activation='relu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = Concatenate()([X1, X2])
		X = Conv2D(256, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		...
		X = Conv2D(409, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = MaxPooling2D(pool_size=3, strides=1, padding='same')(X)

		X1 = Conv2D(409, kernel_size=1, strides=1, activation='relu', padding='same')(X1)
		X = Concatenate()([X, X1])

		X = Flatten()(X)
		X = Dense(294, activation='tanh')(X)
