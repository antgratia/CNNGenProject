...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=6, strides=3, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=4, strides=2, padding='valid')(X)
		X = Conv2D(64, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(128, kernel_size=1, strides=1, activation='relu', padding='valid')(X)
		X = Dropout(0.80)(X)
		X = Conv2D(256, kernel_size=2, strides=1, activation='relu', padding='valid')(X)

		X = GlobalMaxPooling2D()(X)
		X = Dense(153, activation='relu')(X)
		X = Dense(117, activation='tanh')(X)
		X = Dropout(0.10)(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_27.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_27.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_27.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_27_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_27_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_27',
