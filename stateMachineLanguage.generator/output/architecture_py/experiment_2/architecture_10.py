...
try:
	def getModel():
		...
		X = Conv2D(3, kernel_size=4, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(3, kernel_size=6, strides=1, activation='tanh', padding='same')(X)
		X = Dropout(0.80)(X)
		X = MaxPooling2D(pool_size=7, strides=2, padding='same')(X)

		X1 = Conv2D(16, kernel_size=2, strides=2, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(3, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Add()([X, X1])
		...
		X = Conv2D(32, kernel_size=3, strides=1, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)

		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(16, kernel_size=2, strides=1, activation='selu', padding='same')(X1)
		X1 = Conv2D(32, kernel_size=3, strides=1, activation='tanh', padding='valid')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Add()([X, X1])
		X = Dropout(0.10)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
