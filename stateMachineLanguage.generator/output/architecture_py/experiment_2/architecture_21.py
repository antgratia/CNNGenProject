...
try:
	def getModel():
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(16, kernel_size=7, strides=1, activation='selu', padding='same')(X)
		X = Dropout(0.50)(X)
		X = AveragePooling2D(pool_size=2, strides=1, padding='same')(X)

		X1 = Conv2D(16, kernel_size=6, strides=1, activation='relu', padding='same')(X1)
		X = Add()([X, X1])
		...
		X3 = BatchNormalization(epsilon=0.00, axis=3)(X3)
		X3 = Conv2D(32, kernel_size=3, strides=1, activation='tanh', padding='same')(X3)
		X3 = Conv2D(32, kernel_size=3, strides=1, activation='relu', padding='same')(X3)
		X3 = Dropout(0.50)(X3)
		X3 = AveragePooling2D(pool_size=3, strides=1, padding='same')(X3)
		...
		X2 = Conv2D(32, kernel_size=2, strides=1, activation='relu', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(32, kernel_size=3, strides=1, activation='tanh', padding='same')(X2)
		...
		X = Conv2D(128, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(128, kernel_size=2, strides=1, activation='selu', padding='same')(X)

		X2 = X1
		X2 = MaxPooling2D(pool_size=1, strides=1, padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(128, kernel_size=2, strides=1, activation='relu', padding='same')(X2)
		X2 = Conv2D(128, kernel_size=2, strides=1, activation='selu', padding='same')(X2)
		X2 = MaxPooling2D(pool_size=7, strides=1, padding='same')(X2)

		X1 = Add()([X1, X2])

		X = Add()([X, X1])
		X = Conv2D(128, kernel_size=3, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		...
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(128, kernel_size=3, strides=1, activation='selu', padding='same')(X1)
		X = Add()([X, X1])
		X = MaxPooling2D(pool_size=7, strides=1, padding='same')(X)
		X = Conv2D(128, kernel_size=2, strides=1, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		...
		X1 = Conv2D(128, kernel_size=2, strides=1, activation='relu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Add()([X, X1])
		X = Dropout(0.80)(X)
		...
		X = Dense(343, activation='relu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_21.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_21.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_21.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_21_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_21_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_21',
