...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=5, strides=1, activation='selu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.01)(X)
		X = AveragePooling2D(pool_size=7, strides=4, padding='valid')(X)
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(256, kernel_size=1, strides=1, activation='selu', padding='valid')(X)
		X = Dropout(0.01)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(512, kernel_size=1, strides=1, activation='selu', padding='valid')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)

		X = GlobalMaxPooling2D()(X)
		X = Dense(266, activation='relu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_26.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_26.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_26.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_26_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_26_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_26',
