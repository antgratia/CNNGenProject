...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=3, strides=3, activation='relu', padding='same')(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(64, kernel_size=7, strides=1, activation='relu', padding='valid')(X)
		X = Conv2D(128, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(256, kernel_size=5, strides=1, activation='selu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(512, kernel_size=1, strides=1, activation='selu', padding='same')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dropout(0.80)(X)
		X = Dense(235, activation='selu')(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_12.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_12.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_12.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_12_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_12_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_12',
