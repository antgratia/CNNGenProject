...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=4, strides=4, activation='tanh', padding='valid')(X)
		...
		X1 = Conv2D(32, kernel_size=5, strides=1, activation='tanh', padding='same')(X1)
		X = Add()([X, X1])
		...
		X = Conv2D(64, kernel_size=4, strides=1, activation='selu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(128, kernel_size=1, strides=1, activation='selu', padding='valid')(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		...
		X = Concatenate()([X, X1])
		X = Conv2D(358, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = MaxPooling2D(pool_size=4, strides=1, padding='same')(X)

		X = Concatenate()([X, X1])
		X = Conv2D(1288, kernel_size=1, strides=1, activation='relu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dense(257, activation='selu')(X)
		X = Dense(164, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_22.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_22.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_22.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_22_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_22_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_22',
