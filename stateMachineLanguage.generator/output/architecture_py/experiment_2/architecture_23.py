...
try:
	def getModel():
		...
		X = Conv2D(152, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)

		X1 = X
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(152, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = Conv2D(152, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(152, kernel_size=1, strides=1, activation='relu', padding='same')(X1)
		X1 = Dropout(0.10)(X1)
		...
		X = Conv2D(152, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(152, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = Add()([X, X1])
		X = Dropout(0.20)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(304, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)

		X = GlobalMaxPooling2D()(X)
		X = Dense(234, activation='selu')(X)
		X = Dense(180, activation='selu')(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_2/architecture_23.png")
	...
	log_file = open( "../../architecture_log/experiment_2/architecture_23.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_2/architecture_23.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_2/architecture_23_error.log has been create')
	error_file = open("../../architecture_log/experiment_2/architecture_23_error.log" , "w")
	...
	with file:
		...
		# writer.writeheader()
		writer.writerow({'file_name' : 'architecture_23',
