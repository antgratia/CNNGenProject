...
try:
	def getModel():
		...
		X1 = Conv2D(3, kernel_size=3, strides=1, activation='relu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(3, kernel_size=6, strides=1, activation='relu', padding='same')(X1)
		X = Add()([X, X1])
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(3, kernel_size=16, strides=1, activation='relu', padding='valid')(X)

		X1 = Conv2D(16, kernel_size=3, strides=2, activation='relu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(3, kernel_size=2, strides=1, activation='tanh', padding='same')(X1)
		X = Add()([X, X1])
		...
		X1 = Conv2D(32, kernel_size=2, strides=2, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Add()([X, X1])
		...
		X = Dense(7, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_2/architecture_2.png")
	...
	log_file = open( "../../architecture_log/test_2/architecture_2.log", "w")
	...
	print('OK: file ../../architecture_log/test_2/architecture_2.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_2/architecture_2_error.log has been create')
	error_file = open("../../architecture_log/test_2/architecture_2_error.log" , "w")
