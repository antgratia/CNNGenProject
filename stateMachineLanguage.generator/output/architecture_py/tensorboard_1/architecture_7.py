...
try:
	def getModel():
		...
		X = Conv2D(3, kernel_size=6, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.80)(X)
		X = Conv2D(16, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = AveragePooling2D(pool_size=7, strides=5, padding='valid')(X)

		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(16, kernel_size=5, strides=5, activation='relu', padding='valid')(X1)
		X = Add()([X, X1])
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/tensorboard_1/architecture_7.png")
	model.compile(optimizer='adam', loss=keras.losses.sparse_categorical_crossentropy, metrics=['accuracy'])

	es = EarlyStopping(monitor='loss', min_delta=0.001, verbose=1, restore_best_weights=True, patience=7)
	tb = TensorBoard(log_dir="../../architecture_tb/tensorboard_1/architecture_7")
	list_cb = [es, tb]
	start = time()
	...
	log_file = open( "../../architecture_log/tensorboard_1/architecture_7.log", "w")
	...
	print('OK: file ../../architecture_log/tensorboard_1/architecture_7.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/tensorboard_1/architecture_7_error.log has been create')
	error_file = open("../../architecture_log/tensorboard_1/architecture_7_error.log" , "w")
