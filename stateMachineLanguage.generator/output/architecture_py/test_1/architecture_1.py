...
try:
	def getModel():
		...
		X = Conv2D(20, kernel_size=5, strides=3, activation='selu', padding='same')(X)
		...
		X = Conv2D(52, kernel_size=2, strides=2, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.50)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = Conv2D(68, kernel_size=2, strides=2, activation='selu', padding='same')(X)
