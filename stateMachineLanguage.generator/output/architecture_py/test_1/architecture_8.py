...
try:
	def getModel():
		...
		X = Conv2D(20, kernel_size=6, strides=3, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)

		X1 = X
		X = AveragePooling2D(pool_size=3, strides=1, padding='same')(X)
		X = Conv2D(20, kernel_size=3, strides=1, activation='selu', padding='same')(X)
		X = Conv2D(20, kernel_size=4, strides=1, activation='selu', padding='same')(X)
		X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)

		X1 = Conv2D(20, kernel_size=2, strides=1, activation='relu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(20, kernel_size=2, strides=1, activation='relu', padding='same')(X1)
		X = Add()([X, X1])
		X = AveragePooling2D(pool_size=2, strides=1, padding='same')(X)
		X = Conv2D(20, kernel_size=3, strides=1, activation='tanh', padding='same')(X)
		X = Conv2D(52, kernel_size=6, strides=1, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)

		X1 = Conv2D(68, kernel_size=2, strides=2, activation='selu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		...
		X = Conv2D(84, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_1/architecture_8.png")
	...
	log_file = open( "../../architecture_log/test_1/architecture_8.log", "w")
	...
	print('OK: file ../../architecture_log/test_1/architecture_8.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_1/architecture_8_error.log has been create')
	error_file = open("../../architecture_log/test_1/architecture_8_error.log" , "w")
