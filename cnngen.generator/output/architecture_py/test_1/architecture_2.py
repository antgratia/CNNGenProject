...
try:
	def getModel():
		...
		X = Conv2D(20, kernel_size=3, strides=3, activation='relu', padding='valid')(X)
		X = MaxPooling2D(pool_size=6, strides=1, padding='valid')(X)

		X1 = X
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)
		X = Conv2D(20, kernel_size=5, strides=5, activation='tanh', padding='same')(X)

		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(36, kernel_size=5, strides=1, activation='selu', padding='valid')(X1)
		X = Concatenate()([X, X1])
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_1/architecture_2.png")
	...
	log_file = open( "../../architecture_log/test_1/architecture_2.log", "w")
	...
	print('OK: file ../../architecture_log/test_1/architecture_2.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_1/architecture_2_error.log has been create')
	error_file = open("../../architecture_log/test_1/architecture_2_error.log" , "w")
