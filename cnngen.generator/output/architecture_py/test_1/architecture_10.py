...
try:
	def getModel():
		...
		X = Conv2D(3, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)

		X2 = X
		X = AveragePooling2D(pool_size=3, strides=1, padding='same')(X)
		X = Conv2D(3, kernel_size=5, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=7, strides=1, padding='same')(X)

		X = Add()([X, X2])
		X = Conv2D(3, kernel_size=5, strides=3, activation='selu', padding='valid')(X)
		X = Dropout(0.10)(X)

		X1 = Conv2D(20, kernel_size=3, strides=3, activation='tanh', padding='valid')(X1)
		...
		X1 = Conv2D(3, kernel_size=5, strides=2, activation='tanh', padding='valid')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Add()([X, X1])
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(36, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)
		X = Conv2D(52, kernel_size=2, strides=2, activation='relu', padding='valid')(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_1/architecture_10.png")
	...
	log_file = open( "../../architecture_log/test_1/architecture_10.log", "w")
	...
	print('OK: file ../../architecture_log/test_1/architecture_10.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_1/architecture_10_error.log has been create')
	error_file = open("../../architecture_log/test_1/architecture_10_error.log" , "w")
