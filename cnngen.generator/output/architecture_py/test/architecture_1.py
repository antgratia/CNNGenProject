...
tracker = OfflineEmissionsTracker(country_iso_code="BEL", log_level='error', output_file="architecture_1_emissions.csv", output_dir="../../architecture_emissions/test/" )

try:
	def getModel():
		X_input = X = Input([28,28,1])
		X = BatchNormalization(epsilon=1.001E-5, axis=3)(X)
		X = Conv2D(16, kernel_size=5, strides=5, activation='relu', padding='valid')(X)
		...
		X = Conv2D(48, kernel_size=3, strides=1, activation='relu', padding='valid')(X)
		X = Dropout(0.01)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
	...
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test/architecture_1.png")
	...
	log_file = open( "../../architecture_log/test/architecture_1.log", "w")
	...
	print('OK: file ../../architecture_log/test/architecture_1.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test/architecture_1_error.log has been create')
	error_file = open("../../architecture_log/test/architecture_1_error.log" , "w")
	...
	file = open("../../architecture_csv/test/architecture_results.csv", 'a', newline ='')
	with file:
		# identifying header 
