...
try:
	def getModel():
		...
		X = Conv2D(16, kernel_size=5, strides=2, activation='tanh', padding='same')(X)
		X = Dropout(0.80)(X)
		X = MaxPooling2D(pool_size=7, strides=2, padding='valid')(X)
		...
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(50, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		...
		X2 = X
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
		X = Conv2D(60, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)
		...
		X1 = X
		X = Conv2D(100, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(116, kernel_size=1, strides=1, activation='selu', padding='valid')(X)
		...
		X1 = Conv2D(84, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(84, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X = Concatenate()([X, X1])
		...
		X1 = Conv2D(84, kernel_size=1, strides=1, activation='selu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(100, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X = Concatenate()([X, X1])
		X = Dropout(0.01)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_2/architecture_4.png")
	...
	log_file = open( "../../architecture_log/test_2/architecture_4.log", "w")
	...
	print('OK: file ../../architecture_log/test_2/architecture_4.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_2/architecture_4_error.log has been create')
	error_file = open("../../architecture_log/test_2/architecture_4_error.log" , "w")
