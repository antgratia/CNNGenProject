...
try:
	def getModel():
		...
		X = Conv2D(16, kernel_size=2, strides=2, activation='selu', padding='valid')(X)
		X = AveragePooling2D(pool_size=6, strides=2, padding='valid')(X)
		...
		X3 = Conv2D(16, kernel_size=3, strides=1, activation='selu', padding='same')(X3)
		X = Concatenate()([X, X3])
		X = Conv2D(26, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.20)(X)
		...
		X = Concatenate()([X, X2])
		X = Conv2D(42, kernel_size=4, strides=1, activation='tanh', padding='same')(X)
		X = Dropout(0.10)(X)
		...
		X = Conv2D(42, kernel_size=5, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.40)(X)
		...
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(42, kernel_size=2, strides=1, activation='relu', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)
		X2 = Conv2D(42, kernel_size=1, strides=1, activation='selu', padding='same')(X2)
		X = Concatenate()([X, X2])
		X = MaxPooling2D(pool_size=3, strides=1, padding='same')(X)

		X = Concatenate()([X, X1])
		X = Dropout(0.40)(X)
		X = Conv2D(84, kernel_size=2, strides=2, activation='relu', padding='same')(X)
		X = Dropout(0.10)(X)
		X = Conv2D(100, kernel_size=2, strides=1, activation='relu', padding='valid')(X)
		X = Dropout(0.10)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(116, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='same')(X)

		X = GlobalMaxPooling2D()(X)
		X = Dense(74, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_2/architecture_7.png")
	...
	log_file = open( "../../architecture_log/test_2/architecture_7.log", "w")
	...
	print('OK: file ../../architecture_log/test_2/architecture_7.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_2/architecture_7_error.log has been create')
	error_file = open("../../architecture_log/test_2/architecture_7_error.log" , "w")
