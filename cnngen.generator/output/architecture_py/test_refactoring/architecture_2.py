...
tracker = OfflineEmissionsTracker(country_iso_code="BEL", log_level='error', output_file="architecture_2_emissions.csv", output_dir="../../architecture_emissions/test_refactoring/" )
...
try:
	def getModel():
		...
		X = Conv2D(16, kernel_size=2, strides=2, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=1.1E-5, axis=3)(X)
		X = AveragePooling2D(pool_size=7, strides=1, padding='same')(X)

		X1 = Conv2D(16, kernel_size=2, strides=2, activation='selu', padding='same')(X1)
		X = Add()([X, X1])
		...
		X1 = Conv2D(32, kernel_size=6, strides=2, activation='selu', padding='valid')(X1)
		...
		X1 = Conv2D(48, kernel_size=2, strides=2, activation='selu', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.001, axis=3)(X1)
		X = Add()([X, X1])
		...
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
		X = Conv2D(96, kernel_size=1, strides=1, activation='selu', padding='same')(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='same')(X)
	...
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_refactoring/architecture_2.png")
	...
	log_file = open( "../../architecture_log/test_refactoring/architecture_2.log", "w")
	...
	print('OK: file ../../architecture_log/test_refactoring/architecture_2.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_refactoring/architecture_2_error.log has been create')
	error_file = open("../../architecture_log/test_refactoring/architecture_2_error.log" , "w")
