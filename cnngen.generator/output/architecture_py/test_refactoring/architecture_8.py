...
tracker = OfflineEmissionsTracker(country_iso_code="BEL", log_level='error', output_file="architecture_8_emissions.csv", output_dir="../../architecture_emissions/test_refactoring/" )
...
try:
	def getModel():
		...
		X = Conv2D(1, kernel_size=2, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.001, axis=3)(X)
		X = Dropout(0.10)(X)
		X = Conv2D(1, kernel_size=5, strides=1, activation='relu', padding='same')(X)

		X2 = X
		X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=1.1E-7, axis=3)(X)
		X = Conv2D(1, kernel_size=7, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.10)(X)
		X = BatchNormalization(epsilon=0.001, axis=3)(X)
		X = Conv2D(1, kernel_size=3, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.80)(X)

		X3 = X
		X = AveragePooling2D(pool_size=3, strides=1, padding='same')(X)
		X = Conv2D(1, kernel_size=4, strides=1, activation='relu', padding='same')(X)
		...
		X = AveragePooling2D(pool_size=4, strides=1, padding='same')(X)
		X = Conv2D(1, kernel_size=7, strides=1, activation='relu', padding='same')(X)
		X = Conv2D(1, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		...
		X = BatchNormalization(epsilon=0.001, axis=3)(X)
		X = Dropout(0.40)(X)
		X = Conv2D(1, kernel_size=6, strides=1, activation='relu', padding='same')(X)
		...
		X = BatchNormalization(epsilon=1.1E-7, axis=3)(X)
		X = Conv2D(32, kernel_size=2, strides=2, activation='relu', padding='valid')(X)
	...
	plot_model(model, show_shapes=True, to_file="../../architecture_img/test_refactoring/architecture_8.png")
	...
	log_file = open( "../../architecture_log/test_refactoring/architecture_8.log", "w")
	...
	print('OK: file ../../architecture_log/test_refactoring/architecture_8.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/test_refactoring/architecture_8_error.log has been create')
	error_file = open("../../architecture_log/test_refactoring/architecture_8_error.log" , "w")
