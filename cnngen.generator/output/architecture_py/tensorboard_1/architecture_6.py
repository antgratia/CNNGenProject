...
try:
	def getModel():
		...
		X = Conv2D(48, kernel_size=4, strides=4, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.40)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = Conv2D(64, kernel_size=1, strides=1, activation='tanh', padding='valid')(X)

		X1 = X
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)
		X = Conv2D(64, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(80, kernel_size=2, strides=2, activation='selu', padding='same')(X)

		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(64, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(80, kernel_size=2, strides=2, activation='tanh', padding='same')(X1)
		X = Add()([X, X1])
