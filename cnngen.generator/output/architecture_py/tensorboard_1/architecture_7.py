...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=4, strides=2, activation='relu', padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Dropout(0.10)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(48, kernel_size=5, strides=3, activation='relu', padding='valid')(X)
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dropout(0.01)(X)
		X = Dense(261, activation='selu')(X)
