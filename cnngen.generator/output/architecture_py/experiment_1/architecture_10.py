...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=2, strides=1, activation='relu', padding='valid')(X)
		...
		X = Conv2D(204, kernel_size=3, strides=3, activation='selu', padding='valid')(X)
		X = Dropout(0.40)(X)
		X = Conv2D(408, kernel_size=2, strides=1, activation='relu', padding='valid')(X)
		X = MaxPooling2D(pool_size=2, strides=1, padding='same')(X)
		X = Conv2D(816, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
		X = MaxPooling2D(pool_size=2, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(1632, kernel_size=1, strides=1, activation='selu', padding='valid')(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dense(326, activation='tanh')(X)
		X = Dense(133, activation='relu')(X)
		X = Dense(57, activation='selu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/experiment_1/architecture_10.png")
	...
	log_file = open( "../../architecture_log/experiment_1/architecture_10.log", "w")
	...
	print('OK: file ../../architecture_log/experiment_1/architecture_10.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/experiment_1/architecture_10_error.log has been create')
	error_file = open("../../architecture_log/experiment_1/architecture_10_error.log" , "w")
