...
try:
	def getModel():
		...
		X2 = Dropout(0.20)(X2)
		X2 = Conv2D(16, kernel_size=6, strides=1, activation='relu', padding='same')(X2)
		X2 = BatchNormalization(epsilon=0.00, axis=3)(X2)

		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(16, kernel_size=1, strides=1, activation='relu', padding='same')(X1)
		X1 = Conv2D(16, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		...
		X2 = Conv2D(16, kernel_size=3, strides=1, activation='selu', padding='same')(X2)
		...
		X = Conv2D(44, kernel_size=6, strides=6, activation='relu', padding='same')(X)
		X = Dropout(0.20)(X)
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(88, kernel_size=1, strides=1, activation='relu', padding='valid')(X)
		X = AveragePooling2D(pool_size=2, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(176, kernel_size=4, strides=1, activation='selu', padding='valid')(X)
		X = Conv2D(352, kernel_size=2, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = AveragePooling2D(pool_size=2, strides=1, padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(704, kernel_size=2, strides=1, activation='relu', padding='same')(X)
		X = Dropout(0.50)(X)
		X = AveragePooling2D(pool_size=2, strides=1, padding='valid')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(1408, kernel_size=1, strides=1, activation='selu', padding='same')(X)

		X = GlobalAveragePooling2D()(X)
		X = Dropout(0.80)(X)
		X = Dense(281, activation='relu')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../architecture_img/architecture_8.png")
	...
	log_file = open( "../architecture_log/architecture_8.log", "w")
	...
	print('OK: file ../architecture_log/architecture_8.log has been create')
...
# something go wrong
	...
	print('error: file ../architecture_log/architecture_8_error.log has been create')
	error_file = open("../architecture_log/architecture_8_error.log" , "w")
