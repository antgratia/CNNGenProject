...
try:
	def getModel():
		...
		X = Conv2D(32, kernel_size=4, strides=3, activation='relu', padding='valid')(X)
		...
		X1 = Conv2D(128, kernel_size=2, strides=1, activation='tanh', padding='same')(X1)
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(128, kernel_size=2, strides=1, activation='selu', padding='same')(X1)
		X = Add()([X, X1])
		...
		X = Dropout(0.20)(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(128, kernel_size=2, strides=1, activation='relu', padding='same')(X)
		...
		X = Conv2D(409, kernel_size=1, strides=1, activation='tanh', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(409, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(409, kernel_size=1, strides=1, activation='relu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = MaxPooling2D(pool_size=3, strides=1, padding='same')(X)
		...
		X1 = BatchNormalization(epsilon=0.00, axis=3)(X1)
		X1 = Conv2D(409, kernel_size=1, strides=1, activation='tanh', padding='same')(X1)
		X = Concatenate()([X, X1])
	...
	model = getModel()
	plot_model(model, show_shapes=True, to_file="../architecture_img/architecture_4.png")
	...
	log_file = open( "../architecture_log/architecture_4.log", "w")
	...
	print('OK: file ../architecture_log/architecture_4.log has been create')
...
# something go wrong
	...
	print('error: file ../architecture_log/architecture_4_error.log has been create')
	error_file = open("../architecture_log/architecture_4_error.log" , "w")
