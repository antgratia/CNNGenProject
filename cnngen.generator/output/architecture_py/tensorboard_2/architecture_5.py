...
try:
	def getModel():
		...
		X = Conv2D(16, kernel_size=7, strides=4, activation='tanh', padding='same')(X)
		X = Dropout(0.50)(X)
		X = MaxPooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = Conv2D(32, kernel_size=2, strides=1, activation='tanh', padding='valid')(X)
		...
		X1 = Conv2D(48, kernel_size=2, strides=2, activation='selu', padding='valid')(X1)
		X = Concatenate()([X, X1])
		X = AveragePooling2D(pool_size=1, strides=1, padding='valid')(X)
		X = Conv2D(72, kernel_size=2, strides=1, activation='selu', padding='same')(X)
		X = BatchNormalization(epsilon=0.00, axis=3)(X)
		X = Conv2D(72, kernel_size=2, strides=1, activation='relu', padding='same')(X)
		...
		X = Dense(146, activation='tanh')(X)
		X = Dense(10, activation='softmax')(X)
		model = Model(inputs=X_input, outputs=X)
		return model

	model = getModel()
	plot_model(model, show_shapes=True, to_file="../../architecture_img/tensorboard_2/architecture_5.png")
	model.compile(optimizer='adam', loss=keras.losses.sparse_categorical_crossentropy, metrics=['accuracy'])

	es = EarlyStopping(monitor='loss', min_delta=0.001, verbose=1, restore_best_weights=True, patience=7)
	tb = TensorBoard(log_dir="../../architecture_tb/tensorboard_2/architecture_5")
	list_cb = [es, tb]
	start = time()
	...
	log_file = open( "../../architecture_log/tensorboard_2/architecture_5.log", "w")
	...
	print('OK: file ../../architecture_log/tensorboard_2/architecture_5.log has been create')
...
# something go wrong
	...
	print('error: file ../../architecture_log/tensorboard_2/architecture_5_error.log has been create')
	error_file = open("../../architecture_log/tensorboard_2/architecture_5_error.log" , "w")
